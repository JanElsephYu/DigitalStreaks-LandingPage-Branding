...
# We also apply Digital Streaks colors: Teal (#0E6A82), Orange (#FF5722)
custom_css = """
<style>
    ...
    /* Primary CTA Button Styling (Streamlit button override) */
...
with col2:
    if st.button("BOOK YOUR FREE STRATEGY CALL NOW"):
...
audit_q1 = st.radio("When a lead visits your online profiles, do they instantly know why you are better than the competition?", 
                    ["Select an answer...", "Yes, my messaging is crystal clear.", "No, it's a bit generic.", "I'm not sure."])

audit_q2 = st.radio("Are your visual assets (logo, colors, graphics) consistent across every platform?", 
...
if audit_q1 != "Select an answer..." and audit_q2 != "Select an answer...":
    ...
    if st.button("Discuss My Audit Results"):
...
vibe_choice = st.selectbox("Select a Brand Archetype:", 
...
if vibe_choice != "Choose a style...":
    ...
    if st.button(f"Book a Consultation to Build Your {vibe_choice} Brand"):
         st.success("Redirecting to Calendar...")
...
# --- FINAL CTA & FOOTER ---
st.markdown("<h2 style='text-align: center; color: #0E6A82;'>Ready to Make Your Mark Today?</h2>", unsafe_allow_html=True)
